...
def test_positive_get_api_list():
    api_list = helpers.get_api_list(data_dir="./", mock=True)
    assert api_list
    assert "test123" in api_list


def test_positive_get_ver_list():
    ver_list = helpers.get_ver_list(api_name="test123", data_dir="./", mock=True)
    assert ver_list
...
def test_negative_get_ver_list():
    assert not helpers.get_ver_list(api_name="negative", data_dir="./", mock=True)


def test_positive_get_latest():
    assert helpers.get_latest(api_name="test123", data_dir="./", mock=True) == "2.1"


def test_negative_get_latest():
    assert not helpers.get_latest(api_name="negative", data_dir="./", mock=True)
...
def test_negative_get_previous():
    assert not helpers.get_previous("test123", version="1.3", data_dir="./", mock=True)
